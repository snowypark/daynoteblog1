...
<div id="wrapper">
	<div id="content" class="container">
		...
		<div class="board-title">
			...
			<c:if test="${param.category eq 'study'}">개발</c:if>
			<c:if test="${param.category eq 'diary'}">일기</c:if>
		...
		<c:choose>
			<c:when test="${boards.content.size() > 0}">
				<c:choose>
					...
					<c:when test="${param.category eq 'diary'}">
...
<div class="pagination-wrapper">
  ...
  <ul class="pagination justify-content-center">
    ...
    <li class="page-item <c:if test='${boards.totalPages <= startPage + 5}'>disabled</c:if>">
      <a class="page-link" href="/board?category=${param.category}&page=${startPage + 5}&sort=${param.sort}&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}">></a>
    </li>
  </ul>
</div>
